...
class SenderBalanceDetailsExport implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize, WithColumnFormatting
{
    ...
    {
        $q = SenderBalance::query()
            ->with(['sender:id,first_name,last_name', 'admin:id,name'])
        ...
        // auth guard (admin can export any; non-admin only self)
        if (!$this->isAdmin && $this->user && $this->user->getAuthIdentifier() !== $this->userId) {
            // force empty result for safety
            $q->whereRaw('1=0');
        }
    ...
    {
        return [
            'Date (Asia/Baghdad)', 'Status', 'Amount (USD)',
            'Sender (transfer person)', 'Admin', 'Note',
        ];
    }
    ...
    public function map($row): array
    {
        return [
            ...
            (string) $row->status,
            (float) $row->amount,
            $row->sender ? trim(($row->sender->first_name ?? '').' '.($row->sender->last_name ?? '')) : '—',
            $row->admin->name ?? '—',
            $row->note ?? '—',
        ];
    }
    ...
    {
        return [
            'C' => NumberFormat::FORMAT_NUMBER_00, // Amount (USD)
        ];
    }
}
