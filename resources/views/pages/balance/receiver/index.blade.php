...
<style>
  ...
  .balance-page-hero--receiver {
    background:
      linear-gradient(135deg, rgba(5, 150, 105, 0.96), rgba(6, 182, 212, 0.88)),
...
@section('app')
  <div class="page-breadcrumb">
    <div class="row">
      <div class="col-12 align-self-center">
        <h4 class="page-title text-truncate text-dark font-weight-medium mb-1">{{ __('Registers (Receiver)') }}</h4>
  ...
  <div class="container-fluid balance-page-shell">
    <section class="balance-page-hero balance-page-hero--receiver">
      <div>
        <span class="balance-page-hero__kicker">{{ __('Balance Register') }}</span>
        <h4>{{ __('Registers (Receiver)') }}</h4>
        <p>{{ __('Monitor receiver register balances, drill into ledger history, and manage top-up or deduction flows from a cleaner dashboard.') }}</p>
      </div>

      <div class="balance-page-hero__meta">
        <span class="balance-page-hero__pill">{{ __('Currency') }}: IQD</span>
        <span class="balance-page-hero__pill">{{ __('Ledger Scope') }}: {{ __('Receiver Registers') }}</span>
      </div>
    </section>

    @livewire('balance.receiver-balance-livewire')
  </div>
@endsection
